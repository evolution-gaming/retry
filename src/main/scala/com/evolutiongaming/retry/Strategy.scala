...
object Strategy {
   ...
   *
   * The strategy will not schedule a new retry if expected duration will exceed the `max` value.
   */
   ...
   *
   * The deadline is not inclusive, i.e. it will not perform the task at, exactly, the instant
  ...
  implicit class StrategyOps(val self: Strategy) extends AnyVal {
    ...
    /**
     * delay1 = delay * fraction delay1 * random + delay - delay1 See
